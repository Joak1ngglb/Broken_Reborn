...
{
    private readonly Dictionary<Guid, FishBase> _changed = new();
    private FishBase? _editorItem;
    ...
    private void RefreshFolderList()
    {
        ...
        foreach (var folder in FishBase.Lookup.Values.Select(f => f?.Folder ?? string.Empty).Where(f => !string.IsNullOrEmpty(f)).Distinct().OrderBy(s => s))
        {
    ...
    {
        var items = FishBase.Lookup
            .Select(pair => new KeyValuePair<Guid, KeyValuePair<string, string>>(pair.Key,
                new KeyValuePair<string, string>(pair.Value?.Name ?? FishBase.Deleted, pair.Value?.Folder ?? string.Empty)))
            .ToArray();

        var folders = FishBase.Lookup.Values
    ...
    {
        if (!FishBase.TryGet(id, out _editorItem))
        {
            _editorItem = null;
        }
    ...
    private void toolStripItemUndo_Click(object? sender, EventArgs? e)
    {
        ...
        if (DarkMessageBox.ShowWarning(Strings.FishEditor.UndoPrompt, Strings.FishEditor.UndoTitle, DarkDialogButton.YesNo, Icon) == DialogResult.No)
        {
            return;
        }
    ...
    private void toolStripItemDelete_Click(object? sender, EventArgs? e)
    {
        ...
        if (DarkMessageBox.ShowWarning(Strings.FishEditor.DeletePrompt, Strings.FishEditor.DeleteTitle, DarkDialogButton.YesNo, Icon) == DialogResult.Yes)
        {
    ...
    protected override void GameObjectUpdatedDelegate(GameObjectType type)
    {
        ...
        {
            InitEditor();
            if (_editorItem != null && !FishBase.Lookup.Values.Contains(_editorItem))
            {
                _editorItem = null;
                UpdateEditor();
            }
    ...
    private void InitializeComponent()
    {
        ...
        lblSpeed.Text = Strings.FishEditor.Speed;
        ...
        lblHooks.Text = Strings.FishEditor.Hooks;
