...
public partial class FishEventClient
{
    ...
    public bool TryFishing()
    {
        ...
        if (mapAttribute != null)
        {
            ...
            else
            {
                ...
                PacketSender.SendCancelFishing();
                isFishing = false;
                stage = 0;
                if (isDebaging)
    ...
    private bool IsFishingRod()
    {
        ...
        if (weaponSlotIndex > -1 && weaponSlotIndex < player.Equipment.Length)
        {
            if (player.MyEquipment[weaponSlotIndex] < Options.Instance.Player.MaxInventory)
            {
                var itemId = Guid.Empty;
                var slot = player.MyEquipment[weaponSlotIndex];
                if (slot > -1)
    ...
    private void FishingSuccess()
    {
        ...
        {
            eventUI.RedrawIcon(ItemDescriptor.Get(fish.ItemId));
            eventUI.HideBars();
            eventUI.ShowSuccess();
        }
        timerResultUI = Timing.Global.MillisecondsUtc + timerFishingRod;
