    ...
    public static bool LogTransaction(Guid shopId, Guid shopItemId, string buyerName, int quantity, int unitPrice)
    {
        for (var attempt = 0; attempt < MaxConcurrencyRetries; attempt++)
        {
            ...
            catch (DbUpdateConcurrencyException exception)
            {
                Log.Warning(
                    exception,
                    "Player shop transaction concurrency conflict for shop {ShopId} (attempt {Attempt}/{MaxAttempts})",
                    shopId,
                    attempt + 1,
    ...
    public static bool TryPayout(Guid shopId, out long pendingGold)
    {
        ...
        for (var attempt = 0; attempt < MaxConcurrencyRetries; attempt++)
        {
            ...
            catch (DbUpdateConcurrencyException exception)
            {
                Log.Warning(
                    exception,
                    "Player shop payout concurrency conflict for shop {ShopId} (attempt {Attempt}/{MaxAttempts})",
                    shopId,
                    attempt + 1,
                    MaxConcurrencyRetries
                );
