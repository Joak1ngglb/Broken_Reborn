...
namespace Intersect.Server.AI.Pets
{
    ...
    public sealed class PetAIController : IPetBrain
    {
        ...
        {
            var preferDist = GetPreferredAttackDistance();
        ...
        private int GetPreferredAttackDistance()
        {
            var bestRange = _pet
                .GetUsableSpells()
        ...
        {
            if (s?.Combat is not { Friendly: true, TargetType: SpellTargetType.Self } combat)
            {
                return false;
            }

            static bool HasPositiveStatDiff(int[]? diffs, params Stat[] stats)
            {
                if (diffs == null)
                {
                    return false;
                ...
                {
                    var index = (int)stat;
                    if (index >= 0 && index < diffs.Length && diffs[index] > 0)
                    {
                        return true;
                    }
                }
            ...
            if (HasPositiveStatDiff(combat.StatDiff, Stat.Defense, Stat.Vitality) ||
                HasPositiveStatDiff(combat.PercentageStatDiff, Stat.Defense, Stat.Vitality))
            {
                return true;
            }

            return combat.Effect == SpellEffect.Shield || combat.Effect == SpellEffect.Invulnerable;
        }
